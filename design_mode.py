...
PALETTE_COLORS = [
    (180,0,255),(255,0,180),(0,180,255),(255,80,0),(0,255,150),
    (200,0,255),(255,200,0),(0,255,255),(255,60,120),(100,200,255)
]

# 10 vivid color pairs (shadow, glow) for obstacles
OBSTACLE_PALETTE = [
    ((80,0,110),    (190,50,255)),   # violet
    ((0,40,90),     (0,150,255)),    # blue
    ((90,0,0),      (255,60,60)),    # red
    ((0,70,0),      (50,230,80)),    # green
    ((90,40,0),     (255,145,0)),    # orange
    ((60,0,60),     (230,0,230)),    # magenta
    ((0,60,60),     (0,220,200)),    # cyan
    ((70,60,0),     (240,215,0)),    # yellow
    ((75,0,40),     (255,0,130)),    # rose
    ((0,30,70),     (80,135,255)),   # sky blue
]
...
class DesignMode:
    ...
    def __init__(self, snd=None, architect_name="Architect"):
        self.start_point = None
        self.goal_point  = None
        self.obstacles   = []
        self.obstacle_colors = []   # parallel list of (shadow, glow) per obstacle
        ...
        self._inst_bar.set_alpha(175); self._inst_bar.fill((0,0,0))
        self._last_inst_key = None
        self._surf_inst     = None
        self._surf_mode     = None

        # Title bar with architect name
        self._surf_title_bar = self.font_main.render(
            f"ARCHITECT: {architect_name}", True, (0,200,255))
    ...
    def _random_obstacle_color(self):
        return random.choice(OBSTACLE_PALETTE)

    def _place_preset(self, cx, cy):
        name = self.selected_preset
        if name not in PRESET_SHAPES: return
        for dx1, dy1, dx2, dy2 in PRESET_SHAPES[name]:
            seg = ((int(cx+dx1),int(cy+dy1)), (int(cx+dx2),int(cy+dy2)))
            self.obstacles.append(seg)
            self.obstacle_colors.append(self._random_obstacle_color())
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if self.obstacles:
                self.obstacles.pop()
                if self.obstacle_colors: self.obstacle_colors.pop()
                if self.snd: self.snd.play("click")

        elif event.type == pygame.KEYDOWN:
            if event.key == pygame.K_s and not self.saving:
                self.save_map(); self.saving = True
                if self.snd: self.snd.play("click")
    ...
    def update(self):
        ...
        self.palette_hover = -1
        for i, r in enumerate(self._pal_rects):
            if r.collidepoint(mouse): self.palette_hover = i; break
        self.lightning_bolts = [
    ...
    def draw(self, surface):
        ...
        # Ambient lightning — minimized (was 0.02, now 0.004)
        ...
        for i, name in enumerate(self.preset_names):
            ...
            is_hov  = (self.palette_hover == i)
            bg_col  = (60,0,120) if is_sel else (30,0,50) if is_hov else (15,0,30)
            brd_col = (220,0,255) if is_sel else (100,0,200) if is_hov else (60,0,120)
            ...
            scale = 0.18; cx, cy = r.right-28, r.centery
            for dx1,dy1,dx2,dy2 in PRESET_SHAPES[name]:
                pygame.draw.line(surface, col,
                    (int(cx+dx1*scale),int(cy+dy1*scale)),
                    (int(cx+dx2*scale),int(cy+dy2*scale)), 2)

        # Instruction bar (re-render only on change)
        if self.saving:
            inst_key = "saved"; inst_str = "MAP SAVED!  PRESS ENTER TO START"; icol=(0,255,150)
        elif self.start_point is None:
            inst_key = "no_start"; inst_str = "CLICK CANVAS -> SET START POINT"; icol=(255,255,200)
        elif self.goal_point is None:
            inst_key = "no_goal"; inst_str = "CLICK CANVAS -> SET GOAL POINT"; icol=(255,200,100)
        ...
        for i, obs in enumerate(self.obstacles):
            if i < len(self.obstacle_colors):
                shadow_col, glow_col = self.obstacle_colors[i]
            else:
                shadow_col, glow_col = (70,0,90), (200,0,255)
            pygame.draw.line(surface, shadow_col, obs[0], obs[1], 8)
            pygame.draw.line(surface, glow_col,   obs[0], obs[1], 3)
            pygame.draw.circle(surface, glow_col, obs[0], 4)
            pygame.draw.circle(surface, glow_col, obs[1], 4)

        # Lightning flash on fresh placements
        ...
        # Preset preview (reuse surface)
        if self.selected_preset:
            mx2, my2 = pygame.mouse.get_pos()
            if mx2 < self.CANVAS_RIGHT:
                self._preview_surf.fill((0,0,0,0))
                for dx1,dy1,dx2,dy2 in PRESET_SHAPES[self.selected_preset]:
                    pygame.draw.line(self._preview_surf, (200,100,255,130),
                        (int(mx2+dx1),int(my2+dy1)), (int(mx2+dx2),int(my2+dy2)), 3)
